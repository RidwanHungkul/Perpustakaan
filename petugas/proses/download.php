...
$id = $_GET['id'];
$sql = "SELECT peminjaman.*, user.nama_lengkap, buku.judul, perpustakaan.nama_perpus FROM `peminjaman` INNER JOIN user ON peminjaman.user=user.id INNER JOIN buku ON peminjaman.buku=buku.id INNER JOIN perpustakaan ON peminjaman.perpus_id=perpustakaan.id WHERE peminjaman.id=$id";
